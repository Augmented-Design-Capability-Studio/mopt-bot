...
- **Driver preference semantics:** participant-facing solve/evaluate flows should treat `driver_preferences` as explicit-only input. If omitted, default to an empty list (no implicit driver-trait penalties). Canonical/default driver preferences remain valid for dedicated official/canonical evaluation workflows.
