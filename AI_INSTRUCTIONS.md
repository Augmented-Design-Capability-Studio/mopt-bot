...
- **Prompt architecture** (`backend/app/prompts/study_chat.py`):
  ...
  - `STUDY_CHAT_STRUCTURED_JSON_RULES` — JSON response format rules (`assistant_message` + `panel_patch`). Includes **explicit valid/invalid examples** and four strict rules: (1) `"problem"` is the only allowed root key, (2) `weights` must be a JSON object never an array, (3) weight keys must be the exact alias strings, (4) only include changed fields.
  - The system instruction also injects the current panel JSON, compact summaries of the last 4 optimization runs, and hidden researcher steering notes (if any). Steering stays invisible to participants, is treated as highest-priority guidance for the next participant-visible reply, and should be blended in naturally with the prior conversational thread.
- **Framing (participant-visible illusion):** The assistant behaves as a **general metaheuristic optimization** colleague. The **backend** evaluates a **single hard-coded benchmark instance**; the model must **not** name **routing / fleet / scheduling / vehicles / deliveries** domains unless the **user** did so first. **Greetings** and small talk must stay **domain-neutral**. The **actionable** model output is **solver configuration JSON** (`panel_patch`), not source code. Weight keys in `panel_patch` and in the participant panel use **human-readable alias names** (`travel_time`, `deadline_penalty`, `workload_balance`, etc.); the adapter translates these to internal keys before calling the solver.
