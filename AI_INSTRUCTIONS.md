...
Build **two browser frontends** (participant **client** and **researcher**) and **one backend** suitable for a low-cost host (e.g. Raspberry Pi) to run a **user experience study**. The study examines how **workflow** affects optimization outcomes and experience: a **2×2** design (**Novice vs Expert** participants) × (**Agile vs Waterfall** interaction with the AI). In **Agile**, the user need not fully specify the problem up front; the agent may assume missing details, propose configuration or code-like artifacts, and run optimization frequently. In **Waterfall**, the user follows agent guidance to mature the problem formulation before any **RUN**; optimization runs are gated by **intrinsic readiness** (no unanswered open questions, plus a clarification milestone when the open-question list is empty) unless the researcher enables **`optimization_allowed`**. In **Agile**, intrinsic readiness requires meaningful solver configuration (objective weights and/or search/structure signals); the researcher override applies the same way. The underlying task is the VRPTW scenario in `vrptw-problem/`, but the product should **present** as a general metaheuristic-style assistant where required (see §7.3).
...
- **Problem configuration panel (`client/problemConfig/ConfigPanel.tsx`):** instead of a raw JSON textarea, the panel renders `client/problemConfig/ProblemConfigBlocks.tsx` — a structured form under one **Goal terms** section: weight rows (objectives and soft penalties), **structural** fields (max-shift penalty, fixed task→worker locks) without a separate section heading, **Driver Preferences** weight plus a **Preference rules** `<details>` accordion, then a **Search strategy** subheading (algorithm, **algorithm-specific hyperparameters** (`algorithm_params`: e.g. GA `pc`/`pm`, PSO `c1`/`c2`/`w`, etc. — see `backend/app/algorithm_catalog.py` and `client/problemConfig/algorithmCatalog.ts`), **max** iterations as a ceiling with default early stop on cost plateau, optional early-stop toggle and tuning, population, seed). The adapter strips any `algorithm_params` keys not valid for the chosen algorithm; syncing definition from config omits gathered lines for parameters at their catalog defaults. The participant enters **definition edit** or **config edit** by interacting with the surface (fields, **+**/**X** on definition, or first pointer-down on config blocks); while editing, the bottom row shows **Save** / **Cancel** only (Save disabled until there are material changes vs a captured baseline; placeholder-only new rows do not count as dirty). When not editing, **Snapshot** drop-ups offer **Save to snapshot** (`POST /sessions/{id}/snapshots` bookmark of current saved state), **Load from snapshot...**, and on the config tab **From most recent run**; the definition tab also keeps **Sync to config**. The snapshot dialog lists server-stored brief+panel snapshots; restoring triggers chat acknowledgement. The participant form does **not** expose the `only_active_terms` toggle; when omitted, backend scoring defaults to explicit-only objective terms. Researcher controls include a dedicated `only_active_terms` toggle at the session level.
