...
def test_quickbite_optimizer_reserves_exploration_slots_with_many_candidates(monkeypatch):
    captured_instances = []
    ...
    monkeypatch.setattr(
        optimizer_module,
        "encode_greedy_solution",
        lambda orders, locked_assignments=None, rng=None: np.full(VECTOR_LEN, 999.0, dtype=float),
    )

    solver = QuickBiteOptimizer(seed=13)
    candidate_pool = [np.full(VECTOR_LEN, float(i), dtype=float) for i in range(20)]
    solver.solve(
        algorithm="GA",
        epochs=1,
        pop_size=10,
        use_greedy_init=True,
        initial_solutions=candidate_pool,
    )

    starts = captured_instances[0].last_kwargs["starting_solutions"]
    assert len(starts) == 10
    # With greedy enabled, at least two slots are reserved for non-candidates.
    assert sum(1 for v in starts if np.allclose(np.asarray(v), np.full(VECTOR_LEN, 999.0))) >= 1
    assert sum(1 for v in starts if np.allclose(np.asarray(v), np.full(VECTOR_LEN, 999.0))) < 10
