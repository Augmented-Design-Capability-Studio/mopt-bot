...
def test_sync_backfills_search_strategy_when_llm_returns_weights_only(monkeypatch):
    ...
    # The seed only parses structurally-tagged config-* IDs now (see
    # vrptw_problem/brief_seed._extract_structured_slots) — free-text NLP
    # inference was removed.  This brief uses the same row format the
    # panel→brief sync emits during round-trip.
    brief = {
        "items": [
            {
                "id": "config-search-strategy",
                "kind": "gathered",
                "text": "Search strategy: GA (max iterations 100, population size 50).",
                "status": "active",
                "source": "agent",
            },
        ]
