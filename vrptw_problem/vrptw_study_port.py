...
import vrptw_study_bridge
from vrptw_panel_schema import vrptw_panel_patch_response_json_schema
...
def _ensure_vrptw_package_path() -> None:
...
class VrptwStudyPort:
    ...
    def sanitize_panel_config(self, panel_config: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
        return vrptw_study_bridge.sanitize_panel_weights(panel_config)

    def parse_problem_config(self, raw: dict[str, Any]) -> dict[str, Any]:
        return vrptw_study_bridge.parse_problem_config(raw)
    ...
    def solve_request_to_result(
        ...
        return vrptw_study_bridge.solve_request_to_result(body, timeout_sec, cancel_event=cancel_event)

    def derive_problem_panel_from_brief(self, problem_brief: dict[str, Any]) -> dict[str, Any] | None:
        _ensure_vrptw_package_path()
        from vrptw_brief_seed import derive_problem_panel_from_brief as _derive
    ...
    def study_prompt_appendix(self) -> str | None:
        _ensure_vrptw_package_path()
        from vrptw_study_prompts import VRPTW_STUDY_PROMPT_APPENDIX

        return VRPTW_STUDY_PROMPT_APPENDIX

    def config_derive_system_prompt(self) -> str:
        _ensure_vrptw_package_path()
        from vrptw_study_prompts import VRPTW_CONFIG_DERIVE_SYSTEM_PROMPT

        return VRPTW_CONFIG_DERIVE_SYSTEM_PROMPT.strip()

    def panel_patch_response_json_schema(self) -> dict:
        return vrptw_panel_patch_response_json_schema()
