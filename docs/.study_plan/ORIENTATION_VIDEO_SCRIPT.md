...
[Inhale] Now launch a run with the **Run Optimization** button or by asking in chat. [long pause] When it finishes, you'll see the visualization configured by the agent. One tab shows the selected items from your uploaded list while the other is a convergence curve showing how the cost dropped over the search. [pause] You can also expand a panel above to see how each goal contributed to the total cost — useful for spotting which goal is dominating. 

[Inhale] One last thing. If something the agent does doesn't make sense, push back — ask why. [pause] You can also ask the agent general questions about how metaheuristic optimization works; it can answer those just like it can set up your task. [pause] The same workflow you just saw is what you'll use on the QuickBite task next. The agent will configure a more advanced visualization suitable for the problem.
