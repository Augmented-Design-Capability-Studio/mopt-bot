...
>
> Today we'll have you try out a research prototype — an AI-assisted interface for **metaheuristic optimization**, designed to solve a wide range of optimization problems involving trade-offs. Nowadays, even a domain expert may have to hire a programmer to complete such a task. We designed the interface because we believe that very soon, people with or without technical knowledge can use AI programming agents to create their own optimization programs with a fast turnaround.
>
> In the main task of this study, you will try out our interface by role-playing as a scheduler for a small delivery crew. We're not testing you; we're testing the interface. Your honest reactions, including frustrations, are exactly what we want.
>
...
> Great, thanks. Let's pause here for a bit.
...
>
> **First — your role.** In this scenario you are the **scheduler**: the person responsible for the scheduling decisions. You decide priorities and trade-offs based on the briefing materials. You are not expected to write code, and you're not expected to know optimization jargon. Bring whatever you already know about logistics or planning, and use it however feels natural — engage as yourself.
>
> **Second — the AI agent's role.** Treat the agent as an **optimization programmer who is helping you**. Imagine you'd otherwise have to hire a programmer to set up and run the solver — that's the kind of teammate this agent is meant to "replace." So talk to it the way you'd talk to that programmer: tell it what you care about, push back when something doesn't make sense, and ask why it's doing what it's doing.
>
> The thing we're really trying to learn is how people feel about this kind of interface. So as you go, try **thinking aloud**, that is, vocalizing your thoughts for your actions. Notice when the system helps and when it gets in the way — we'll talk about those at the end.
...
## 5. Task briefing handoff (~12 min, script ends)

**[SAY]**

> Now I'll play the actual task briefing. It introduces a small delivery service called **QuickBite** — a fleet of vehicles and a set of orders to deliver in one day. You'll have a hard copy reference on hand for the rest of the session, so don't worry about memorizing anything.
...
**[SAY]** *(after the survey)*
> Now we'll continue the scheduling task. Remember:
> - You are taking the stance of a delivery fleet's scheduler
> - You will message the AI as if it is a optimization programmer
> - We can stop once you obtain a satisfactory schedule or we reach the 30 minute mark; we could go a few minutes over if necessary
> - Try your best to think aloud
> - Ask problem-related questions to the AI first
> - For glitches and procedures questions, feel free to let the researcher know!
...
| Task-briefing handoff (incl. video + reading)              | ~12 min   | 30–34      |
| Interaction                                                | ~35 min   | 60-70      |
| Final questions                                            | ~5 min    | 65-75      |
