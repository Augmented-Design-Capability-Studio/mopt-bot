...
The brief is the single source of truth between chat and panel config. Config is always *derived from* the brief, never written directly. Brief structure includes `goal_summary`, a single rolling `run_summary`, `items` (`kind: "gathered"` or `kind: "assumption"`) with `source` (`user` / `agent` / `upload`), `open_questions` (with `status`/`answer_text`), and a top-level structured `goal_terms` map (same shape as the panel's `goal_terms`). The brief's `goal_terms` is the canonical carrier for solver-config detail across the chat → brief → panel → brief loop; `merge_problem_brief_patch` deep-merges per goal-term key, deep-merges `properties` per property name, and replaces list-valued properties (notably `properties.driver_preferences`) wholesale. Cleanup should consolidate run/session bookkeeping noise into `run_summary` instead of leaving per-run rows in gathered/assumptions/questions. Upload turns (participant and researcher-simulated) also run deterministic open-question reconciliation: upload-related open questions are auto-resolved when upload evidence is present, via a placeholder validator hook that currently always passes and is reserved for future file-content checks.

Per-goal-term structured metadata (e.g. VRPTW puts driver-preference rules at `goal_terms.worker_preference.properties.driver_preferences` and the shift cap at `goal_terms.shift_limit.properties.max_shift_hours`) is owned by the active port. The shared schema layer (`backend/app/problems/schema_shared.goal_term_entry_schema(...)` / `goal_terms_schema(...)`) accepts a per-port `properties` sub-schema; each port returns its own from `StudyProblemPort.goal_term_properties_schema()`. The brief-update LLM response schema is built per-port at call time (`_build_brief_update_response_schema(test_problem_id)` in `services/llm.py`) so Gemini structured output sees a fully typed `properties` object — without this the model emits empty `properties: {}` and rules vanish on the chat side.

**Goal-term evidence anchoring.** Each `goal_terms[key]` entry can carry `evidence_item_ids: list[str]` citing the brief `items[]` ids that justify that term. The brief-update and panel-derive LLMs are instructed to populate this whenever they introduce a new term. On every brief-merge and panel-derive pass, `app.services.goal_term_anchoring.filter_unanchored_new_goal_terms` drops newly-introduced keys whose ids don't resolve to a valid item — workflow-aware (waterfall = only `gathered` items count as evidence; agile/demo also count `assumption`). Existing keys are passed through untouched so retunes never regress. Anchor priority: (1) explicit `evidence_item_ids` cite resolves to a real item, (2) self-anchored properties (worker_preference with non-empty driver_preferences, shift_limit with max_shift_hours set), (3) embedding-cosine fallback against item text (best-effort, threshold 0.55, silently disabled if no API key). `_rebuild_goal_terms_metadata` (`vrptw_problem/study_bridge.py`) preserves `evidence_item_ids` across panel sanitize so the cite trail survives manual retunes.

**Search-strategy grounding.** Separate deterministic gate for algorithm / epochs / pop_size / algorithm_params (panel fields, not under `goal_terms`). `algorithm_mentioned_in_brief` does a case-insensitive substring scan of brief items[] for any of the five canonical algorithm names + aliases (with word-boundary checks on the short ones); when no item names one, the LLM's search-strategy choice is dropped and the current panel values preserved.

After every brief-update merge, `apply_brief_patch_with_cleanup` (`backend/app/routers/sessions/derivation.py`) calls `port.synthesize_brief_items_from_goal_terms(brief.goal_terms)` to render participant-visible prose `gathered` rows from the structured carrier (VRPTW: one `config-driver-pref-{vid}-{discriminator}` row per rule). Stale rows are pruned via id-prefix filtering keyed off `port.prose_id_prefixes_for_goal_term(...)` — id-only, never text inspection. The same panel→brief mirror runs in `sync_problem_brief_from_panel` (`backend/app/problem_brief.py`), so manual UI adds of preference rules immediately surface in the Definition tab.
...
Port hooks for problem-specific structured metadata under `goal_terms[key].properties` (see also the brief layer notes above):

- `goal_term_properties_schema()` returns the JSON schema slotted into the shared `goal_terms_schema(...)` factory at brief-patch and panel-patch construction time. VRPTW returns `VRPTW_GOAL_TERM_PROPERTIES_SCHEMA` from `vrptw_problem/panel_schema.py` (typed `driver_preferences` array + `max_shift_hours`); knapsack/template return `None` (permissive default).
- `synthesize_brief_items_from_goal_terms(goal_terms)` returns participant-visible prose `gathered` items synthesized deterministically from the structured carrier. VRPTW delegates to `vrptw_problem/brief_seed.synthesize_driver_preference_items` and emits one `config-driver-pref-{vid}-{discriminator}` row per rule.
- `prose_id_prefixes_for_goal_term(goal_term_key)` returns the id-prefixes the synthesizer owns so the brief-merge layer can drop stale rows on each refresh without inspecting text. VRPTW returns `("config-driver-pref-",)` for `worker_preference`.
