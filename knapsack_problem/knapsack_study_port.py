...
from knapsack_panel_schema import knapsack_panel_patch_response_json_schema


def _ensure_knapsack_path() -> None:
...
class KnapsackStudyPort:
    ...
    def parse_problem_config(self, raw: dict[str, Any]) -> dict[str, Any]:
        _ensure_knapsack_path()
        from knapsack_study_bridge import parse_problem_config as _parse
    ...
    def solve_request_to_result(
        ...
        _ensure_knapsack_path()
        from mealpy_solve import OptimizationCancelled
        from knapsack_study_bridge import solve_request_to_result as _solve
    ...
    def derive_problem_panel_from_brief(self, problem_brief: dict[str, Any]) -> dict[str, Any] | None:
        _ensure_knapsack_path()
        from knapsack_brief_seed import derive_problem_panel_from_brief
    ...
    def study_prompt_appendix(self) -> str | None:
        _ensure_knapsack_path()
        from knapsack_study_prompts import KNAPSACK_STUDY_PROMPT_APPENDIX

        return KNAPSACK_STUDY_PROMPT_APPENDIX

    def config_derive_system_prompt(self) -> str:
        _ensure_knapsack_path()
        from knapsack_study_prompts import KNAPSACK_CONFIG_DERIVE_SYSTEM_PROMPT

        return KNAPSACK_CONFIG_DERIVE_SYSTEM_PROMPT.strip()

    def panel_patch_response_json_schema(self) -> dict:
        return knapsack_panel_patch_response_json_schema()
