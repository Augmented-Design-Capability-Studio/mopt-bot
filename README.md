...
- **Fast chat path:** `POST /sessions/:id/messages` returns the assistant chat reply first (when **Ask model** is on), then continues brief/config derivation in a background thread. The same response **always** includes a `processing` snapshot (even when `invoke_model` is `false`) so the client never keeps a stale pending overlay after a user-only message. Session JSON exposes `processing` state (`brief_status`, `config_status`, `processing_revision`, `processing_error`) so the frontend can show panel-level pending state without blocking chat. **Pending is marked at the start of the model call**, so any client polling during the model window sees `brief_status` / `config_status` as `pending` and shows a chat response-spinner — this matters when the message originates outside the participant frontend (e.g. researcher **`POST /sessions/{id}/researcher/simulate-participant-upload`**, which routes through the same handler). The patch / skip / interpret-only branches still settle to `ready`/`failed` with a single revision bump per turn (`else` branch reuses the early-mark revision). The participant client's `syncMessages` poll triggers an immediate session sync when new messages arrive, so the spinner appears on the next message-poll tick rather than waiting for the slower session cadence. After the hidden brief pass, if the normalized brief is unchanged, the background thread **skips the config LLM** and uses heuristic `derive_problem_panel_from_brief` only (same shortcut for `agile` and `waterfall`). Optional body field **`skip_hidden_brief_update`** (with `invoke_model`): visible reply only, no background brief merge — used after participant-driven definition/config saves and snapshot restores so the model does not overwrite stored state; open-question answer flows still run full derivation.
...
- **`STUDY_CHAT_SYSTEM_PROMPT`** — domain-neutral, delivery-partner persona with temperature-aware context. Temperature is resolved **model-first** per turn (cold/warm/hot), with deterministic **session-evidence** fallback if classification is unavailable (brief/panel/run state only; no keyword regex matching). **Cold** turns stay problem-agnostic and avoid module specifics; **warm/hot** turns can incorporate active module user docs plus visible config/run context while still hiding internal aliases/keys. When the user asks to "write code" or "implement", the agent updates the problem brief (not source code) and the backend derives solver configuration JSON from that brief. Participant-facing wording defaults to plain language (for example priorities, importance levels, and run settings) before technical optimization terms. In visible chat, internal schema key names should stay hidden unless the participant explicitly requests technical field names, and objective suggestions should avoid "activate/enable/turn on" phrasing in favor of neutral optimization language. Mentioning **MEALpy** is allowed when directly asked or when clarifying solver-library provenance.
- **Participant-safe retrieval + capability framing:** visible chat instruction injects a capabilities block and per-turn reference excerpts from participant-safe docs (`docs/user/*.md`; plus active `*_problem/docs/user/*.md` in warm/hot turns). In **cold** turns, capabilities stay domain-agnostic (no module goal-term or visualization specifics); richer module-aware details appear only in warm/hot turns. Retrieval excludes internal alias disclosures. Execution capability messaging is centralized with extensible modes (`read_only_simulated`, `propose_patch`, `apply_patch`) while current study runtime remains configuration-only.
...
- **Goal-term representation:** `problem.hard_constraints` / `problem.soft_constraints` lists are deprecated. Canonical term semantics live in the `goal_terms` map (each entry: `weight`, `type` ∈ {objective, soft, hard, custom}, optional `rank` / `locked` / `properties`); `weights` + `constraint_types` are projected views maintained by `study_bridge._apply_goal_terms_overlay` / `_rebuild_goal_terms_metadata`. `goal_terms[key].properties` is the per-port slot for structured per-term metadata — VRPTW carries driver-preference rules at `goal_terms.worker_preference.properties.driver_preferences` and the shift cap at `goal_terms.shift_limit.properties.max_shift_hours`. Each port declares the typed shape of `properties` via `StudyProblemPort.goal_term_properties_schema()`; the shared factory `app.problems.schema_shared.goal_terms_schema(...)` slots it into both the panel-patch and brief-patch response schemas (the brief-update LLM response schema is built per-port at call time so Gemini structured output emits real rule objects, not empty `{}`).
- **Brief carries `goal_terms` end-to-end.** The problem brief stores `goal_terms` as a top-level structured field alongside `items` / `open_questions` / `goal_summary` / `run_summary`. `merge_problem_brief_patch` deep-merges per goal-term key, deep-merges `properties` per property name, and replaces list-valued properties (notably `driver_preferences`) wholesale. `sync_problem_brief_from_panel` mirrors `panel.problem.goal_terms` into the brief verbatim, so manual panel-side preference adds are first-class brief data without going through prose. After every chat-turn brief patch, `apply_brief_patch_with_cleanup` calls `port.synthesize_brief_items_from_goal_terms(brief.goal_terms)` to render participant-visible prose `gathered` rows (VRPTW emits one `config-driver-pref-{vid}-{discriminator}` row per rule); stale rows are pruned via id-prefix filtering keyed off `port.prose_id_prefixes_for_goal_term(...)` — id only, never text. The cross-prompt contract for VRPTW lives once in `vrptw_problem/study_prompts.DRIVER_PREFERENCES_BRIEF_CONTRACT` and is imported into both the chat appendix and config-derive prompt.
- **Strict goal-term validation + retries:** panel sync now enforces cross-field validation before persistence. `goal_terms[*].type` is mandatory (`objective|soft|hard|custom`), `goal_term_order` keys must exist, and terms must be grounded in Definition `items` (no hallucinated terms, no missing grounded terms). LLM config derivation auto-retries with validation feedback (bounded attempts), then marks `processing_error` on failure. Participant chat shows a retryable sync error bubble.
...
- **`STUDY_CHAT_WORKFLOW_WATERFALL`** / **`STUDY_CHAT_WORKFLOW_AGILE`** — injected based on `session.workflow_mode`; waterfall encourages full upfront specification, agile encourages frequent short runs. The LLM service (`backend/app/services/llm.py`) selects the right chunk from the session's `workflow_mode`. **Agile** prompts also require **explicit participant agreement** before introducing a **new** solver goal-term (weight) key in the brief, bias **agent-originated** detail into **`assumption`** rows (`source: "agent"`) with sparse **`open_questions`** (zero is OK), and disambiguate generic “priority” language from **express / priority-order SLA** terms; VRPTW **`vrptw_problem/study_prompts.py`** config-derivation text matches (no inventing `express_miss_penalty` from generic punctuality phrasing alone). The Definition **Assumptions** section includes **↑ promote to Gathered** (participant-confirmed) per row.
...
The chat footer includes a simulated **`Upload file(s)...`** action with a horizontal **scrollable chip row** of file names (remove with **×** clears the chip only; chat history is unchanged). Participant-facing copy and the study agent should point users at that **Upload file(s)...** control only. For lab/testing, **`POST /sessions/{id}/researcher/simulate-participant-upload`** (researcher auth) can post the same user-visible upload line, defaulting to **`DRIVER_INFO.csv`** and **`ORDERS.csv`** when the body is empty (researcher UI: **More actions & settings**). Upload messages now deterministically auto-resolve upload-related open questions in the brief (same path for participant uploads and researcher simulated uploads), so stale “please upload files” blockers clear immediately when upload evidence exists. In logistics-style conversations, the assistant should request uploads for order data plus driver information/preferences, acknowledge uploads as if ingested, and may reference city-traffic API assumptions (time-of-day traffic and disruptions) while reasoning about schedules.

When the participant saves the definition, just-answered open questions (`status: "answered"` with non-empty `answer_text`) are routed through a batched **answer classifier** in `PATCH /sessions/{id}/problem-brief` (backend `_route_oq_answers_through_classifier` in `routers/sessions/router.py`, prompt `STUDY_CHAT_OQ_CLASSIFY_TASK`). The classifier rephrases each concrete answer into a one-sentence gathered fact (no more verbose `Question — Answer` joins) and routes hedged answers (e.g. "you decide" / "i don't know") per workflow:
- **Waterfall** — drop the original OQ, raise a *simpler* follow-up `open_question` with 2–4 concrete `choices` (rendered as radio buttons on the OQ card). Never an assumption — preserves the waterfall "no assumptions" invariant.
- **Agile / demo** — drop the OQ, append a `kind: "assumption"` row (`source: "agent"`); the existing agile-mode rule "announce assumptions in visible chat" still applies.

Each just-answered card shows a spinning shield while the classifier runs (`participantOps.processingOqIds`). If the LLM call fails, the legacy `_format_answered_open_question_gathered` / `_promote_answered_open_questions_to_gathered` path runs as a fallback so saves are never blocked. The Definition tab action row (next to **Sync to config**, drop-up like **Snapshot**) includes **⋯** → **Clean up definition**, and immediately below it **Clean up open questions**. The open-question action calls a dedicated backend endpoint that targets only `open_questions`, while the regular cleanup-definition action remains holistic. After chat turns that mutate the brief, backend also runs an automatic open-question cleanup pass (all modes: agile/waterfall/demo) using an LLM-first prune and conservative deterministic fallback. Upload turns additionally run a deterministic upload-question resolver with a placeholder validator hook (`upload_satisfies_open_question`) that currently returns pass; this is the future extension point for real file-content checks. **`merge_problem_brief_patch`** keeps existing open questions when a cleanup patch sets `replace_open_questions` but omits `open_questions` (models must send an explicit list, including `[]`, to replace). The model is instructed not to put resolved answers in `open_questions` (no `(Answered: …)` suffixes); the merge layer also moves those patterns into gathered. `merge_problem_brief_patch` still preserves answered state from the base when a patch references the same question id but omits status/answer fields, before normalization promotes answered rows.

**Brief normalization:** `normalize_problem_brief` splits long compound objective lists and lines starting with **`Constraint handling:`** (several comma- or *and*-separated penalties in one sentence) into **separate gathered rows**, one per term—parallel to compound objective splitting—so each maps cleanly to goal-term / config seeding. Promoted answered-open-question rows (`item-gathered-from-question-*`, or any gathered/assumption line containing an em dash `—` as in `Question — Answer`) are **not** atomized, so commas inside the answer do not split the row.
