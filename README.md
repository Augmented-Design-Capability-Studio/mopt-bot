...
- **Fast chat path:** `POST /sessions/:id/messages` now returns the assistant chat reply first, then continues brief/config derivation in a background thread. Session JSON exposes `processing` state (`brief_status`, `config_status`, `processing_revision`, `processing_error`) so the frontend can show panel-level pending state without blocking chat.
