...
- **Goal-term representation:** `problem.hard_constraints` / `problem.soft_constraints` lists are deprecated. Canonical term semantics are `weights` + `constraint_types` (+ `locked_goal_terms`), with optional `goal_terms` metadata entries (`weight`, `type`, `locked`, optional `properties` such as `max_shift_hours` or `driver_preferences` linkage).
